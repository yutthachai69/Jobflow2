...
export default function SendApprovalButton({ workOrderId, status, approvalToken }: SendApprovalButtonProps) {
    ...
    const handleSend = async () => {
        const confirmed = await confirm({
            title: 'ส่งขออนุมัติ',
            message: 'ต้องการสร้างลิงก์อนุมัติและส่งแจ้งเตือนไปยัง LINE Admin หรือไม่?',
            confirmText: 'ส่งเลย',
            cancelText: 'ยกเลิก',
            variant: 'info',
        })
        ...
        try {
            ...
            if (result.success) {
                toast.success('สร้างรายการและส่งแจ้งเตือนเรียบร้อยแล้ว!', { duration: 4000 })
                router.refresh()
            } else {
            ...
            <>
                ...
                <div className="w-full bg-orange-50 border border-orange-200 rounded-xl p-4">
                    ...
                    <div className="flex gap-2 mt-2">
                        ...
                        <a
                            href={`https://line.me/R/msg/text/?${encodeURIComponent(`รบกวนตรวจสอบและอนุมัติงานซ่อมครับ: ${origin}/approve/${approvalToken}`)}`}
                            target="_blank"
