...
export default function EditRoomForm({ room }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    formData.set('floorId', room.floor.id)
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-8 border border-gray-100">
      ...
      <input type="hidden" name="floorId" value={room.floor.id} />
      ...
      <div className="space-y-6">
        <div>
          ...
          <input
            ...
            className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all duration-200 text-gray-900 ${
        ...
        <div className="flex items-center gap-4 pt-4">
          <button
            ...
            className="flex-1 bg-gradient-to-r from-orange-600 to-amber-600 text-white px-6 py-3 rounded-lg hover:shadow-lg hover:scale-[1.02] font-semibold transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
          >
